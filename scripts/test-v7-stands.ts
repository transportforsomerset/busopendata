...
const compactDataV7: CompactVehicleDataV7 = {
  version: 7,
  dates: inputV4.dates,
...
const outputJson = JSON.stringify(compactDataV7);
...
for (const [operatorCode, operatorVehicles] of Object.entries(compactDataV7.operators)) {
...
const sourceById = new Map(vehiclesV7.map((vehicle) => [vehicle.vehicle_id, vehicle]));
...
if (differences === 0) {
  console.log("✓ All vehicles are identical after v7 stands");
} else {
  console.log(`❌ Total v7 differences: ${differences}`);
}
...
// BEGIN EXPORT FINISH.
      return {
        compactDataV7,
        json: JSON.stringify(compactDataV7),
        differences,
    };
}
// END EXPORT FINISH.
